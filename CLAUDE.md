...
- La CI possède un job séparé d'installation de production : bibliothèques système audio, `pip install -r requirements.txt`, `pip check`, `pip-audit`, puis imports de fumée de Torch, PyAudio, Open Interpreter, spaCy, Kokoro et faster-whisper. Un job `macos-14` rejoue en plus les contrats Apple simulés et les smoke tests natifs (`osascript`, dictionnaires Mail/Calendar/Contacts/Messages, plist launchd, `say`, CoreAudio, iMessage, capture et audio). Les appareils physiques et l'observation 24 h ne sont pas vérifiés.
- Le job `Qualité et audits` exécute `scripts/quality_gate.py` sur le diff : Ruff, Black, mypy, Bandit et règles Semgrep locales. mypy contrôle aussi en permanence `database/time_buckets.py`, `jarvis/events.py`, `pipeline.py` et `security_headers.py`. La suite backend tourne sous `coverage` avec branches et un seuil initial de 35 %, destiné à augmenter avec la résorption de la dette historique.
- `frontend/package.json`, `web/package.json` et les trois jobs Node GitHub Actions épinglent tous `pnpm 11.11.0`. Les deux lockfiles v9 sont installés avec `--frozen-lockfile`, et un contrat pytest interdit désormais toute dérive de version. Les audits npm bloquent les alertes élevées sur les frontends actifs ; `GHSA-qwww-vcr4-c8h2` est temporairement exclu parce qu'ils sont exportés statiquement sans React Server Components/Server Actions. Le fallback `pwa/` historique bloque les alertes critiques, ses alertes élevées restant une dette de migration documentée.
...
5. **En-têtes de sécurité absents ou contournés par les erreurs d'authentification anticipées** — CSP, nosniff, X-Frame-Options, Referrer-Policy et HSTS conditionnel sont appliqués globalement, y compris aux réponses 401/403/428 du verrou.
