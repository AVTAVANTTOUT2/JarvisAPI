...
# Domaines techniques → Cursor
_TECH_PATTERNS = (
    r"\b(code|bug|debug|stack\s*trace|crash|exception|traceback)\b",
    r"\b(pull\s*request|PR\b|commit|branche|branch|merge|rebase|git)\b",
    r"\b(migration|sqlite|schema|dockerfile|docker|ci\b|pytest|unittest)\b",
    r"\b(refactor|compile|build|deploy|release|apk|gradle)\b",
    r"\b(frontend|backend|android|api\s*router|endpoint)\b",
    r"\b(corrige|fixe|répare|repare|implémente|implemente|ajoute\s+une?\s+page)\b.*\b(code|fichier|projet|dépôt|depot|jarvis)\b",
    r"\b(cursor|worktree|self[-\s]?repair|auto[-\s]?réparation|auto[-\s]?reparation)\b",
...
_BRIEFING_PATTERNS = (
    r"\b(briefing|fais[- ]moi\s+le\s+point|quoi\s+de\s+neuf|résumé\s+du\s+matin|resume\s+du\s+matin)\b",
    r"\b(version\s+courte|seulement\s+les\s+urgences|qu['']est[- ]ce\s+qui\s+a\s+changé)\b",
)
...
class CognitiveRouter:
    ...
    def route(
        ...
        *,
        ...
        is_tech = _matches_any(folded, _TECH_PATTERNS)
        is_explain = _matches_any(folded, _EXPLAIN_PATTERNS)
        if is_tech and is_explain and not _matches_any(
            folded,
        ...
        # 3) Exécution technique → Cursor
        if is_tech or _matches_any(
            folded,
            (
                r"\b(corrige\s+le\s+bug|ajoute\s+une\s+page|crée\s+la\s+migration|cree\s+la\s+migration)\b",
                r"\b(travaille\s+sur|modifie\s+le\s+code|dans\s+le\s+dépôt|dans\s+le\s+depot)\b",
            ),
        ):
