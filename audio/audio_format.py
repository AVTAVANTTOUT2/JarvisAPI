...
# Type MIME annoncé aux clients qui reçoivent un fichier (web, mobile,
# appareils distants). La pile locale produit du PCM16 encapsulé en WAV : un
# seul conteneur, sans encodeur externe, lisible par tous les navigateurs.
#
# Cette constante remplace une table par moteur. Elle n'était nécessaire que
# parce que chaque moteur rendait un conteneur différent (MP3 pour Edge, M4A
# pour `say`, WAV pour les moteurs locaux) — et se tromper d'annonce empêchait
# purement et simplement la lecture côté client.
DEFAULT_TTS_MIME = "audio/wav"
