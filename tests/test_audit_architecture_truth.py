...
def test_real_repo_smoke_counts_stable() -> None:
    ...
    # Vague 2B : +1 table `location_point_dedup` (idempotence batch GPS)
    assert tables["counts"]["persistantes_post_init"] == 71
    assert tables["counts"]["physiques_max_default_fts_on"] == 76
