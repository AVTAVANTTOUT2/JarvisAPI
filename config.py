...
INGESTION_IMESSAGE_WATCH_ENABLED = (
    _get("INGESTION_IMESSAGE_WATCH_ENABLED", "true").lower() == "true"
)
...
AGENTIC_WRITE_CONCURRENCY = _positive_int("AGENTIC_WRITE_CONCURRENCY", 1)
AGENTIC_MAX_BACKGROUND_RUNS = _positive_int("AGENTIC_MAX_BACKGROUND_RUNS", 1)
# Mac mini M4 32 Go : garder ~2 Go libres avant d'admettre un nouveau run.
AGENTIC_MIN_FREE_MEMORY_MB = _positive_int("AGENTIC_MIN_FREE_MEMORY_MB", 2048)
AGENTIC_MAX_QUEUE_WAIT_S = _positive_int("AGENTIC_MAX_QUEUE_WAIT_S", 120)
AGENTIC_PROVIDER_CRASH_COOLDOWN_S = _positive_int(
    "AGENTIC_PROVIDER_CRASH_COOLDOWN_S", 30
)
AGENTIC_WORKTREE_TTL_HOURS = _positive_int("AGENTIC_WORKTREE_TTL_HOURS", 72)
AGENTIC_WORKTREE_MAX_COUNT = _positive_int("AGENTIC_WORKTREE_MAX_COUNT", 8)
AGENTIC_WORKTREE_MAX_TOTAL_BYTES = _positive_int(
    "AGENTIC_WORKTREE_MAX_TOTAL_BYTES", 2 * 1024 * 1024 * 1024
)
# Identifiants de modèles extraits du catalogue provider, jamais inventés.
AGENTIC_MODEL_FAST = _get("AGENTIC_MODEL_FAST", "").strip()
AGENTIC_MODEL_CODING = _get("AGENTIC_MODEL_CODING", "").strip()
...
SUPERVISOR_RESTART_LOCK_TIMEOUT_S = _positive_float(
    "SUPERVISOR_RESTART_LOCK_TIMEOUT_S", 20.0
)
