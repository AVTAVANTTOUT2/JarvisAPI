...
`apple-music-mcp` n'était monté que dans le runtime agentique après
`media:publish`.
...
4. Le runtime agentique **garde** le même binaire pour les vrais workflows
   `media:publish` (transcode, ffmpeg). Le chat Cursor MCP n'est pas le chat
   JARVIS.
